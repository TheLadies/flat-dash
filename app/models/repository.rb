...
class Repository < ActiveRecord::Base
  ...
  def self.todays_pull_requests
    pull_counts_array = []
    pull_dates = Repository.select(:user_login, 'MAX(pull_updated_at) AS last_updated', 'COUNT(id) AS num_pull_requests').where("pull_updated_at > ?", 1.days.ago).group(:user_login)
    pull_dates.each do |repo|
